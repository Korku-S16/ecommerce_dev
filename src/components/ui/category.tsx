import React from "react";
import {
  FaMobileAlt,
  FaTshirt,
  FaBlender,
  FaMagic,
  FaTv,
  FaGamepad,
} from "react-icons/fa";

const categories = [
  { label: "Mobiles & Tablets", icon: <FaMobileAlt /> },
  { label: "Tv & Appliances", icon: <FaTv /> },
  { label: "Fashion", icon: <FaTshirt /> },
  { label: "Kitchen", icon: <FaBlender /> },
  { label: "Beauty", icon: <FaMagic /> },
  { label: "Others", icon: <FaGamepad /> },
];

const Category = () => {
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <h2 className="text-xl ml-6 font-semibold mb-6 text-left text-gray-800">
        Browse By Category
      </h2>
      <div className="flex flex-wrap gap-4 justify-center">
        {categories.map((cat, index) => (
          <div
            key={index}
            className="flex flex-col items-center justify-center bg-gray-100 rounded-lg px-6 py-10 w-56 hover:bg-gray-200 transition"
          >
            <div className="text-3xl text-black mb-2">{cat.icon}</div>
            <span className="text-sm text-center text-black font-medium">
              {cat.label}
            </span>
          </div>
        ))}
      </div>
    </div>
  );
};

export default Category;
